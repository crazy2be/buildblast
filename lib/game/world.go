...
func NewWorld(seed float64, announce func(message string)) *World {
	...
	w.EntitiesObserv.OnAdd(w, func(entityID string, entity Entity) {
		entity.GetStatus().OnChanged(w, func(status Status) {
			if status.StatusFlag == Status_Dead {
				entity.Respawn(w.FindSpawn())
				entity.GetStatus().Set(Status{
		...
		entity.GetHealth().OnChanged(w, func(health Health) {
			if health.Points <= 0 {
				entity.GetStatus().Set(Status{
...
func (w *World) TeamChanged(key string, team Team) {
	if team.Points < w.KOTH_CONSTS.MaxPoints.Get() {
		return
	}
