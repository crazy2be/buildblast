...
ALSO
	- For client side stuff simulate a variety of bulk of commands, to insure we handle overloading the system and to make sure our performance doesn't degrade too badly.
	- For server side stuff simulate lag in some manner, and perhaps also simulate lots of activity by other players.
	- Perhaps add in passive checks to determine the delay in data changing and then that new data being drawn on the screen.